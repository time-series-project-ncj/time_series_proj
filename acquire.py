...
def merge_dataframes():
    ...
    with open('2018-05-27_through_2018-06-26.csv') as fd:
        reader=csv.reader(fd)
        interestingrows=[row for idx, row in enumerate(reader) if idx in range(36,67)]
    ...
    with open('2018-07-28_through_2018-08-26.csv') as fd:
        reader=csv.reader(fd)
        interestingrows=[row for idx, row in enumerate(reader) if idx in range(35,65)]
    ...
    with open('2018-11-28_through_2018-12-28.csv') as fd:
        reader=csv.reader(fd)
        interestingrows=[row for idx, row in enumerate(reader) if idx in range(36,45)]
