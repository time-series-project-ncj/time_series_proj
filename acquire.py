...
def merge_activities():
    ...
    df_final = df_final.rename(columns={0: "date", 1: 'calories_burned', 2: 'steps', 3: 'distance', 4: ' floors', 5: 'minutes_sedentary', 6: 'minutes_lightly_active',7: 'minutes_fairly_active', 8: 'minutes_very_active',9: 'activity_calories'})
...
#     with open('2018-05-27_through_2018-06-26.csv') as fd:
#     reader=csv.reader(fd)
#     interestingrows=[row for idx, row in enumerate(reader) if idx in range(2,33)]
#     df = pd.DataFrame(interestingrows)
#     df1 = pd.DataFrame(interestingrows)
#     with open('2018-06-27_through_2018-07-27.csv') as fd:
#     reader=csv.reader(fd)
...
#     frames = [df, df1, df2, df3, df4, df5, df6, df7]
#     df_final = pd.concat(frames)
#     df_final = df_final.rename(columns={0: "date", 1: 'calories_in'})
#     df_final.drop([0], inplace=True)
#     return df_final
